...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        UINavigationBar.appearance().barTintColor = UIColor(red:0.07, green:0.64, blue:0.67, alpha:1.0) //aqua
    ...
    func getManagedObjectContext(completionHandler: @escaping (_ context: NSManagedObjectContext) -> ()) {
        ...
        if let docsDir = fm.urls(for: .documentDirectory, in: .userDomainMask).first {
            let url = docsDir.appendingPathComponent("dataDoc")
                ...
                if (fileExists) {
                    document.open(completionHandler: { (success:Bool) in
                        if (success) {
                            ...
                            print("failed to get managedObjectContext - doc did not open")
                        }
                    })
                }
                else {
                        ...
                        if (success) {
                            document.open(completionHandler: { (success:Bool) in
                                if (success) {
                                    ...
                                    print("failed to get managedObjectContext - doc id not open")
                                }
                            })
                        }
                        else {
                            print("failed to get managedObjectContext - doc did not save")
                        }
                    }
                }
            }
